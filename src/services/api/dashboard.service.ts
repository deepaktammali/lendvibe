...
export const dashboardService = {
  async getDashboardSummary(): Promise<DashboardSummary> {
    ...
    const totalOutstandingBalance = loansWithBalances.reduce((sum, loan) => sum + loan.real_remaining_principal, 0);
    ...
    const upcomingPayments = loans
      ...
      .map(loan => ({
        ...loan,
        borrower_name: 'Unknown', // Would need to join with borrower data
        days_until_due: Math.ceil((new Date(loan.end_date!).getTime() - now.getTime()) / (1000 * 60 * 60 * 24)),
      }))
      .slice(0, 5);
  ...
  async getDashboardStats(): Promise<DashboardStats> {
    ...
    const paymentStats = {
      ...
      lastMonth: payments.filter(payment =>
    ...
    const borrowerStats = {
      ...
      newThisMonth: borrowers.filter(borrower =>
        new Date(borrower.created_at) >= thisMonth
      ).length,
    };

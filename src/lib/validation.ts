...
export const loanSchema = z.object({
  ...
  term_months: z.number().int().min(1, 'Term must be at least 1 month').max(360, 'Term cannot exceed 360 months'),
...
export const fixedIncomeSchema = z.object({
  ...
  income_rate: z.number().min(0, 'Income rate cannot be negative').max(100, 'Income rate cannot exceed 100%'),
